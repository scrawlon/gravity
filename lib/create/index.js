...
async function start(project) {
  Gravity.Utils.spinner.text = chalk.yellow(`Initializing Project...`);
  Gravity.Utils.spinner.start();

  const currentWorkingDirectory = Gravity.Utils.getCurrentWorkingDirectory(project);

  /* Fail if project directory already exists */
  if ( Gravity.Utils.directoryExists(`${currentWorkingDirectory}${path.sep}${project}`) ) {
    Gravity.Utils.spinner.fail(chalk.red(`Install directory (${project}) already exists.\n`));
    process.exit(0);
  }

  const tempDirectory = Gravity.Utils.getTempDirectory(project);
  const bitbucketUsername = Gravity.Utils.helpers.bitbucket.getUsername();

  if ( tempDirectory && bitbucketUsername && bitbucketApiUrl ) {
    Gravity.Utils.spinner.succeed(`Project initialized.`);
  }

  // console.log({bitbucketApiUrl});
  // console.log({bitbucketUsername});

  // ------------------------------------------------------------------------------------ //
  // Prompt user for Build Tools and JS preference
  // ------------------------------------------------------------------------------------ //
  const projectConfig = await require('./projectConfig')(project);

  Gravity.Utils.saveProjectConfig(projectConfig, tempDirectory);

  console.log({tempDirectory, projectConfig});

  /* Create/Confirm Bitbucket upstream/origin */
  if ( projectConfig.repoExists ) {
    const repoName = projectConfig.repoUpstreamUrl;
  } else {
    const repoName = projectConfig.repoNewName;
  }

  Gravity.Utils.spinner.text = chalk.yellow(`Creating/Confirming Bitbucket repos...`);
  Gravity.Utils.spinner.start();

  if ( projectConfig.installExists ) {
    /* Download db and uploads from WP Engine install */
