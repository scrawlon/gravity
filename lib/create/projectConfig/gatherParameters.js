...
const bitbucketUpstream = 'git@bitbucket.org:sinclairdigitalsolutions'
    ...
    inquirer.prompt([
      {
        ...
        message: `Upstream repo url? (${bitbucketUpstream}...)`,
        validate: (input) => {
          const answer = input.trim();
          const validUpstreamRepo = /^git@bitbucket.org:sinclairdigitalsolutions.*\.git$/.test(answer)
          const repoExists = Gravity.Utils.helpers.bitbucket.doesRepoExist(answer);
          ...
          if (!repoExists) {
            return `Can't connect to ${answer}. Double check URL and try again.`
          }
        ...
        validate: (input) => {
          const answer = input.trim();
          const hyphenizedRepoName = Gravity.Utils.hyphenize(answer.toLowerCase());
          const validRepoNewName = /^[A-Za-z0-9 _-]+$/.test(answer);
          const repoExists = Gravity.Utils.helpers.bitbucket.doesRepoExist(`${bitbucketUpstream}/${hyphenizedRepoName}`);

          if (!validRepoNewName) {
            return `"${answer}" is invalid. Use only letters, numbers and hyphens.`
          }

          if (repoExists) {
            return `Bitbucket repo name "${hyphenizedRepoName}" is already in use. Try again.`
          }
        ...
        name: 'install',
        message: 'What is the install name? (alphanumeric)',
        validate: (input) => {
          ...
          if (answer.length < 9) {
            return `"${answer}" is too short. Use 9-15 characters.`;
          }

          if (answer.length > 15) {
            return `"${answer}" is too long. Use 9-15 characters.`;
          }
        ...
        type: 'confirm',
        name: 'projectConfirmed',
        message: (response) => {
          return `Ready to create this project? \n`
            + chalk.yellow(`Bitbucket Repo: ${response.repoNewName ? response.repoNewName : response.repoUpstreamUrl} \n WP Engine Install: ${response.install} \n ${response.woocommerce ? 'WooCommerce' : ''}`);
        }
      }, {
        type: 'confirm',
        name: 'cancelProjectCreate',
        message: (response) => {
          Gravity.Utils.spinner.fail(`Project Create canceled.\n Type "gravity create ${project}" to start over.`);
          process.exit(0);
        },
        when: (response) => {
          return !response.projectConfirmed;
        }
      // }, {
      //   type: 'confirm',
      //   name: 'starter',
