    ...
    inquirer.prompt([
      {
        ...
        validate: (input) => {
          const answer = Gravity.Utils.hyphenize(input.trim().toLowerCase());
          const validRepoNewName = /^[A-Za-z0-9 _-]+$/.test(answer);
          const repoExists = Gravity.Utils.helpers.bitbucket.doesRepoExist(`${bitbucketUpstream}/${answer}`);
          ...
          if (repoExists) {
            return `Bitbucket repo name "${answer}" is already in use. Try again.`
          }
