...
function getCurrentWorkingDirectory(project) {
  ...
  if ( isTestMode(currentWorkingDirectory) ) {
    ...
    return `${currentWorkingDirectory}/test`;
  }
...
function saveProjectConfig(projectConfig, tempDirectory) {
  ...
  fs.writeFile(`${tempDirectory}/config.json`, jsonString, 'utf8', (err) => {
    if (err) {
      Gravity.Utils.spinner.fail(chalk.red('Could not save config file: ' + err));
      process.exit(0);
    };
  })
}
...
function isTestMode(currentWorkingDirectory) {
  ...
  return currentPackageDirectory === `${currentWorkingDirectory}/`;
}
