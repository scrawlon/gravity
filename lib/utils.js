...
/**
 * Checks to make sure that user can connect to required services
 */
function checkConnections() {
  const connectionTypes = Object.keys(connections);

  connectionTypes.forEach(function(connectionType) {
    const connection = connections[connectionType];

    exec(`${connection.command}`, (err, stdout, stderr) => {
      if ( err ) {
        spinner.fail(`Cannot connect to ${connection.name}\n`);

        console.error(
          chalk.red(`${tab}Make sure you're logged into ${connection.name} and try again.\n`)
        )
  ...
  return new Promise(resolve => {
    setTimeout(() => {
      spinner.succeed('Verified Connections.');
      resolve();
    }, 1000);
  });
}
...
//
//   return execSync(`${connection.getUserCommand}`, {encoding: 'utf8'}, (err, stdout, stderr) => {
//     if ( err ) {
//       spinner.fail(`Cannot connect to ${connection.name}\n`);
//
//       console.error(
